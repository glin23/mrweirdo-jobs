...
- **ONLY** when a stream run (`shared/stream_run.mjs`, onboard Step 5) dispatches a row whose `ats_platform == 'greenhouse'` to this skill, after the pre-dispatch guard re-read the ledger for that row.
- The invocation form is implicit (apply_batch runs the driver once per row the stream run dispatches).
