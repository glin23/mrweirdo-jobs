...
- Push (arnold-ops，关卡 11): 文档提交 `6fde653`（父 `4450766`，仅 restart-apply TASK/VERIFY_REPORT 两文件）；fetch 无分叉，非强推；origin/main `11352e4` → `6fde653`。本行未提交。
- Round 17 (arnold-builder，verify 第 8 轮 P3): `f4a5fce` 启动时读完系统证书 + NODE_OPTIONS --import 预加载进所有 node 子进程（safe_exit 引入即生效，入口补到 12 个）。红测试：替身子进程成功 exit(0) 旧码 19/200 SIGSEGV → 0/200，未捕获异常 → 200/200 退出码 1。全量连跑 3 次 488/488，CI 另三步 exit 0。子进程启动点 13 处 / process.exit 44 个文件清单写进 BUILD_NOTES。未推，本行未提交。
- Round 18 (arnold-builder，lead 复验 f3aef0f RACE 偶发 ENOENT): 根因 = 胜方 cleanOldRuns 把败方刚建、未上锁的运行目录当残留删掉。`4d34c78` 只清「建目录的 start 进程已不在」的目录。RACE 循环 100 次：修前 8/100 → 修后 0/100；全量连跑 3 次 489/489，CI 另三步 exit 0。未推，本行未提交。
- Round 20 lead 独立复验 tip `437441c`（含 f4a5fce 启动预加载证书、4d34c78 RACE 只清已退出进程的目录）：stream_run_lock 循环 40 次 0 失败；全量连跑 4 次 489/489（每次日志分存）；CI 另三步 0。此前两次复验各抓到 1 次偶发（emit_outcome_flush SIGSEGV、RACE ENOENT），均已由 builder 定根因修复。
- Round 20: verify 第 9 轮中途 stall（600s 无进展，看门狗判失败，无结论）。按铁律 5 未自动重试，待拍板人决定是否重派。f4a5fce..437441c 未推（缺 verify 可推判定）。
